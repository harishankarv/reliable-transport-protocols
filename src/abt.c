...
struct pkt packet_a; //packet to be sent A to B
struct pkt ack_b; //ack to be sent from B to A

//maintain 2 state variables at A and B. 
//A has 4 states: state 0/2 = Waiting for call 0/1 from above. state 1/3 = Waiting for call ack 0/1 
//B has 2 states: state 0: Waiting for call 0 from below. State 1: Waiting for call 1 from below
  ...
  struct msg message;
{
  ...
  printf("[Sender] [A_output] state is: %d\n", state_a);
  printf("[Sender] [A_output] received data from upper layer: %.*s \n", 20, message.data);
  
  //generate packet and start timer

  bzero(&packet_a, sizeof(packet_a));

  // if we are in state 0, seqnum is 0, else it is 1
  packet_a.seqnum  = (state_a == 0)? 0 : 1;
  memcpy(&packet_a.payload, message.data, 20);
  packet_a.checksum = calc_checksum(packet_a);
  
  printf("[Sender] [A_output] Application data generated: %.*s \n", 20, packet_a.payload);
  
  tolayer3(0, packet_a);
  starttimer(0, TIMEOUT);

  printf("[Sender] [A_output] Packet %d sent.\n", packet_a.seqnum);
  
  A_transport++;

  //as packet is sent, go to next state in sequence.
  state_a = (state_a == 0)? 1: 3;
  printf("[Sender] [A_output] Going to state %d\n", state_a);
  ...
  struct pkt packet;
{
  ...
  printf("[Sender] [A_input] State is %d\n", state_a);
  printf("[Sender] [A_input] ACK received: %d\n", packet.acknum);
 
  int checksum = calc_checksum(packet);
  printf("[Sender] [A_input] Calculated Checksum: %d\n", checksum);
  printf("[Sender] [A_input] Received Checksum: %d\n", packet.checksum);
  
  // if checksum does not match return
  if (checksum != packet.checksum)
  {
    printf("[Sender] [A_input] Checksum does not match. Received ACK is corrupt\n");
    return;
  }
  ...
  {
    printf("[Sender] [A_input] Wrong ACK %d received.\n", packet.acknum);
  ...
  printf("[Sender] [A_input] Packet % d received successfully at Receiver\n", packet.acknum);
  stoptimer(0);
  state_a = (state_a == 1)? 2 : 0;
  printf("[Sender] [A_input] Going to state %d\n", state_a);
  return;
...
void A_timerinterrupt()
{
  ...
  printf("[Sender] [A_timerinterrupt] Resending .. %.*s \n", 20, packet_a.payload);
  ...
  struct pkt packet;
{
  ...
  printf("[Receiver] [B_input] State: %d \n", state_b);  
  printf("[Receiver] [B_input] Packet received: %.*s \n", 20, packet.payload);
  printf("[Receiver] [B_input] Seqnum: %d\n", packet.seqnum);

  int checksum = calc_checksum(packet);
  printf("[Receiver] [B_input] Calculated Checksum: %d\n", checksum);
  printf("[Receiver] [B_input] Received Checksum: %d\n", packet.checksum);

  bzero(&ack_b, sizeof(ack_b));

  // if we get the wrong packet from the one we are expecting, resend precious ACK

  if ( (state_b == 0 && packet.seqnum != 0) || (state_b == 1 && packet.seqnum != 1) )
  {
    printf("[Receiver] [B_input] Sequence number %d received. Does not match. \n", packet.seqnum);
    
    ack_b.acknum = (state_b == 0)? 1: 0;

    ack_b.checksum = calc_checksum(ack_b);
    printf("[Receiver] [B_input] Resending ACK %d\n", ack_b.acknum);
    tolayer3(1, ack_b);  
  ...
  if (checksum != packet.checksum)
  {
    printf("[Receiver] [B_input] Checksum does not match. Received packet is corrupt \n");
  ...
  printf("[Receiver] [B_input] Packet received: %.*s \n", 20, packet.payload);
  printf("[Receiver] [B_input] Sequence number: %d \n", packet.seqnum);
  printf("[Receiver] [B_input] Checksum matches.\n");

  tolayer5(1, packet.payload);
  B_application++;

  printf("[Receiver] [B_input] Data handed over to application layer. Count: %d\n", B_application);
 
  ack_b.acknum = (state_b == 0)? 0 : 1;
  
  ack_b.checksum = calc_checksum(ack_b);
  printf("[Receiver] [B_input] Sending ACK %d\n", ack_b.acknum);
  tolayer3(1, ack_b);

  state_b =  (state_b == 0)? 1 : 0; 
  printf("[Receiver] [B_input] Going to state: %d", state_b);
